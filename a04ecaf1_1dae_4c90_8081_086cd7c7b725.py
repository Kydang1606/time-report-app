...
def export_report(df, config, output_file_path):
    ...
    try:
        ...
        for project in df['Project name'].unique():
            ...
            # Thống kê cuối tuần theo tháng của dự án
        ...
        print(f"❌ Lỗi thực tế trong export_report: {e}")
        traceback.print_exc()
        return False

def export_pdf_report(df, config, pdf_report_path, logo_path):
    if not pdf_report_path:
        raise ValueError("❌ pdf_report_path is empty.")
    ...
    try:
        ...
        success, msg = create_pdf_from_charts_comp(charts_for_pdf, pdf_report_path, "TRIAC TIME REPORT - STANDARD", pdf_config_info, logo_path)
        return success
    except Exception as e:
        traceback.print_exc()
        return False
        ...
        if 'Year' in df.columns and 'MonthName' in df.columns:
            ...
            for i, project in enumerate(projects):
                ...
                for j, val in enumerate(y_vals):
                    if val > 0:
                        ax.annotate(f"{val:.0f}", xy=(x[j] + i * width, val), xytext=(0, 5), textcoords="offset points", ha='center', fontsize=8, rotation=90)
            ...
            if not df_task.empty:
                ...
                for container in bars.containers:
                    for bar in container:
                        ...
                        if height > 0:
                            ax.annotate(f"{height:.0f}", xy=(bar.get_x() + bar.get_width() / 2, height), xytext=(0, 5), textcoords="offset points", ha='center', fontsize=8, rotation=90)
...
def export_comparison_pdf_report(df_comparison, comparison_config, pdf_file_path, comparison_mode, logo_path, filter_mode="Total"):
    if 'Hours' not in df_comparison.columns:
        raise ValueError("❌ Column 'Hours' is missing.")    
...
def generate_comparison_pdf_report(df_comparison, comparison_config, pdf_file_path, comparison_mode, logo_path, filter_mode="Total"):
    ...
    try:
        ...
        charts_dict = create_comparison_chart(df_comparison, comparison_mode, chart_title, x_label, y_label, chart_path_placeholder, comparison_config, filter_mode)
        ...
        if charts_dict:
            ...
            for key in ["time", "total", "task", "workcentre"]:
                if key == "time" and only_one_time_point: continue
...
def apply_comparison_filters(df_raw, comparison_config, comparison_mode, filter_mode="Total"):
    ...
    if df_filtered.empty:
        return pd.DataFrame(), f"Không tìm thấy dữ liệu cho chế độ so sánh.", []

    if comparison_mode in ["So Sánh Dự Án Trong Một Tháng", "Compare Projects in a Month"]:
        if len(years) != 1 or len(months) != 1 or len(selected_projects) < 2:
            return pd.DataFrame(), "Vui lòng chọn MỘT năm, MỘT tháng và ít nhất HAI dự án.", []
        ...
        if 'Task' not in df_comparison.columns: df_comparison['Task'] = 'All'
        if 'Workcentre' not in df_comparison.columns: df_comparison['Workcentre'] = 'All'
        ...
        return df_comparison, f"So sánh giờ giữa các dự án trong {months[0]}, năm {years[0]}", selected_projects

    elif comparison_mode in ["So Sánh Dự Án Trong Một Năm", "Compare Projects in a Year"]:
        if len(years) != 1 or len(selected_projects) < 2:
            return pd.DataFrame(), "Vui lòng chọn MỘT năm và ít nhất HAI dự án.", []
        ...
        df_total_row = pd.DataFrame([{'Project Name': 'Total', **{col: df_comparison[col].sum() for col in existing_months + ['Total Hours']}}])
        ...
        return df_comparison, f"So sánh giờ giữa các dự án trong năm {years[0]}", selected_projects
...
def export_comparison_report(df_comparison, comparison_config, output_file_path, comparison_mode, filter_mode="Total"):
    try:
        ...
        with pd.ExcelWriter(output_file_path, engine='openpyxl') as writer:
            ...
            ws.cell(row=info_row+1, column=1, value="Năm: " + ', '.join(map(str, comparison_config.get('years', []))))
            ws.cell(row=info_row+2, column=1, value="Tháng: " + ', '.join(comparison_config.get('months', [])))
            ws.cell(row=info_row+3, column=1, value="Dự án: " + ', '.join(comparison_config.get('selected_projects', [])))

            # Vẽ biểu đồ Excel So sánh
            if not df_comparison.empty:
                chart = BarChart()
                chart.title = "Báo cáo so sánh"
                chart.x_axis.title = "Dự án/Thời gian"
                chart.y_axis.title = "Giờ"
                
                # Loại bỏ dòng Total trước khi kéo vùng tham chiếu vẽ chart
                ...
                if 'Total Hours' in df_comparison.columns:
                    ...
                    data_ref = Reference(ws, min_col=col_idx, min_row=2, max_row=max_r)
                    cats_ref = Reference(ws, min_col=1, min_row=2, max_row=max_r)
